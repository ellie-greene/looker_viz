  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      if (!measureFields.length || !pivots || !pivots.length) {
        container.innerHTML = '<div style="color:#888; font-family: Google Sans, Roboto, sans-serif;">No pivoted data found. Pivot on your period dimension and add a count measure.</div>';
        done();
        return;
      }
      ...
      // Fallback: if no period dimension, just use first row as current
      if (!currentRow && data.length >= 1) currentRow = data[0];
      if (!previousRow && data.length >= 2) previousRow = data[1];
 
      // Use first measure field for values
      ...
      pivots.forEach(function(pivot) {
        ...
        // Build the cell key — Looker uses "pivotkey|fieldname" format
        var cellKey = pivotKey + '|' + measureField.name;
 
        var currentCell = currentRow && currentRow[cellKey];
        var previousCell = previousRow && previousRow[cellKey];
 
        var currentValue = currentCell
          ? (currentCell.rendered || currentCell.value || 0)
          : '—';
 
        var ppLine = '';
        if (previousCell && previousCell.value !== null && previousCell.value !== undefined) {
          var curr = currentCell ? (currentCell.value || 0) : 0;
          var prev = previousCell.value || 0;
          if (prev !== 0) {
        ...
        card.innerHTML =
          '<div style="font-size:0.85em; color:#696969; margin-bottom:8px; font-weight:500; text-transform:capitalize;">' + pivotLabel + '</div>' +
