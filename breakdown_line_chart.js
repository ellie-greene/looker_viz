looker.plugins.visualizations.add({
  ...
  updateAsync: function(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      // Find whichever dimension contains 'current period' / 'previous period'
      var allDims = (fields.dimensions || []).concat(fields.dimension_like || []);
      var dimField    = null;
      var currentRow  = null;
      var previousRow = null;

      allDims.forEach(function(candidate) {
        data.forEach(function(row) {
          var cell = row[candidate.name];
          if (!cell) return;
          var val = (cell.value || '').toString().toLowerCase();
